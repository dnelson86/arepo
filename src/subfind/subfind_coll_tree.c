...
int subfind_coll_treebuild_construct(int npart, struct unbind_data *mp)
{
  ...
  SubTree_IntPos_list = (unsigned long long *) mymalloc_movable(&SubTree_IntPos_list, "SubTree_IntPos_list", 3 * NumPart * sizeof(unsigned long long));
      ...
      for(j = 0; j < 3; j++)
        {
          ...
          if(*posp < SubDomainCorner[j] || *posp >= SubDomainCorner[j] + SubDomainLen)
            {
              terminate("out of box i=%d j=%d coord=%g SubDomainCorner=(%g|%g|%g) SubDomainLen=%g", i, j, *posp, SubDomainCorner[0], SubDomainCorner[1], SubDomainCorner[2], SubDomainLen);
            }
              ...
              if(SubTreeAllocFactor > MAX_TREE_ALLOC_FACTOR)
                {
                  char buf[500];
                  sprintf(buf,
                          "task %d: looks like a serious problem for particle %d, stopping with particle dump.  SubTree_NumNodes=%d SubTree_MaxNodes=%d  0=%d NumPart=%d\n",
          ...
          for(k = 0; k < 2; k++)
            {
              ...
              if(subfind_coll_create_empty_nodes(SubTree_NextFreeNode - 1, SubTopNodes[topnode].Daughter + sub, bits + 1, 2 * x + i, 2 * y + j, 2 * z + k, xxc, yyc, zzc, ilen) < 0)
                return -1;
            }
    }
          ...
          for(k = 0; k < 2; k++)
            {
              ...
              SubTree_NextFreeNode++;
              subfind_coll_treeupdate_toplevel(SubTree_NextFreeNode - 1, SubTopNodes[topnode].Daughter + sub, bits + 1, 2 * x + i, 2 * y + j, 2 * z + k);
            }
