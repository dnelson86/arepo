  ...
  for(int i = 0; i < ncpu; i++)
    {
      ...
      while((work + work_before
             + (end + 1 < ndomain ?
                fac_work * leaf[end + 1].workgrav + fac_load * DomainLeaveNode[end + 1].Count + fac_worksph * leaf[end + 1].worksph : 0)
...
void domain_combine_multipledomains(void)
{
          ...
          if(domain_to_be_balanced[bin])
            {
              ...
              if(bin >= P[i].TimeBinGrav)
                ...
                {
                  if(domain_bintolevel[bin] >= 0)
                    loc_bin_Cost[bin * ndomains + n].GravCost += MIN_FLOAT_NUMBER + domain_grav_weight[bin] * P[i].GravCost[domain_bintolevel[bin]];
                  else
                    {
                      if(domain_refbin[bin] >= 0)
                        loc_bin_Cost[bin * ndomains + n].GravCost += MIN_FLOAT_NUMBER + domain_grav_weight[bin] * P[i].GravCost[domain_bintolevel[domain_refbin[bin]]];
                      else
  ...
  allreduce_sparse_double_sum((double *) (loc_bin_Cost + All.LowestOccupiedTimeBin * ndomains),
                              (double *) (glob_bin_Cost + All.LowestOccupiedTimeBin * ndomains), 2 * ndomains * (All.HighestOccupiedTimeBin - All.LowestOccupiedTimeBin + 1));
  ...
  for(int n = 0; n < ndomains; n++)
    {
      domainAssign[n].normalized_load =
        domainAssign[n].work / (tot_work + MIN_FLOAT_NUMBER) + domainAssign[n].worksph / (tot_worksph + MIN_FLOAT_NUMBER) + domainAssign[n].load / ((double) tot_load + MIN_FLOAT_NUMBER);
    }
              ...
              if(set == 0)
                {
                  ...
                  for(np = RB_MIN(mytree, &queue_gravcost[bin]), rep = 0; np != NULL && rep < n_lowest; np = RB_NEXT(mytree, &queue_gravcost[bin], np), rep++)
                    candidates[rep] = np->target;
                }
              else
                {
                  for(np = RB_MIN(mytree, &queue_hydrocost[bin]), rep = 0; np != NULL && rep < n_lowest; np = RB_NEXT(mytree, &queue_hydrocost[bin], np), rep++)
                    candidates[rep] = np->target;
                }
          ...
          if(domain_to_be_balanced[bin])
            {
              ...
              double eps_grav = 1.0e-9 * (domainAssign[n].load / totpartcount) * totgravcost;   /* these will be added in order to break degeneracies in the sort-order in case the grav/hydro cost in certain cells is zero */
