  ...
  if(n_b > 0)
    {
      ...
      if(fabs((omega - All.Omega0) / omega) > 1.0e-3 || fabs((omega_b - All.OmegaBaryon) / omega_b) > 1.0e-3)
        if(ThisTask == 0)
          warn("I've found something odd! The mass content accounts for Omega=%g and OmegaBaryon=%g, but you specified Omega=%g and OmegaBaryon=%g in the parameterfile.", omega, omega_b, All.Omega0,
               All.OmegaBaryon);
